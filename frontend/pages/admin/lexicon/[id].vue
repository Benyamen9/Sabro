...
const mayPropose = computed(() => canPropose('Lexicon'))
...
const status = computed<'loading' | 'unauthorized' | 'failed' | 'notFound' | 'ready'>(() => {
  ...
  // whether this person is staff at all, the Lexicon grant says whether this is
  // one of their areas.
...
<template>
  <section class="mx-auto max-w-3xl">
    ...
    <template v-else-if="entry">
      ...
      <h1 class="mb-6 mt-2 font-serif text-3xl font-semibold tracking-[-0.015em]">{{ t('admin.lexicon.form.editTitle') }}</h1>
      ...
      <!-- A reviewer sees the entry but cannot save it. Saying so up front beats a
           form that looks editable until the server refuses it. -->
      <p
        v-if="!mayEdit"
        class="mb-5 rounded-md border border-[var(--color-border-strong)] bg-[var(--color-bg-subtle)] px-4 py-3 font-sans text-sm text-[var(--color-text)]"
      >{{ t('admin.lexicon.readOnlyNotice') }}</p>

      <LexiconEntryForm
        :key="`${entry.updatedAt}:${proposalId ?? ''}`"
        :prefill="prefill"
        :entry="entry"
      ...
      <!-- Lifecycle. Publishing, the playable pool, the recording and deletion are
           all editor actions; a reviewer proposes instead. -->
