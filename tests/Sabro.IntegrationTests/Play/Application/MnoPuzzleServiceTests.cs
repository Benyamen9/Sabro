...
// mno_daily_puzzles table never collides on the unique (game, date) key.
// Generation is random by design; the tests assert the persisted-and-shared
// contract (get-or-create) and the served puzzle's invariants, not a specific
// equation.
...
public class MnoPuzzleServiceTests
{
    ...
    public async Task GetTodaysPuzzle_FirstCall_GeneratesAndPersists()
    {
        ...
        var result = await NewService(ctx, today).GetTodaysPuzzleAsync(ct);
    ...
    public async Task GetTodaysPuzzle_SecondCallSameDay_ReturnsSameEquation()
    {
        ...
        await using (var ctx = fixture.CreatePlayContext())
        {
            var first = await NewService(ctx, today).GetTodaysPuzzleAsync(ct);
            firstExpression = first.Value!.Expression;
        }

        await using var ctx2 = fixture.CreatePlayContext();
        var second = await NewService(ctx2, today).GetTodaysPuzzleAsync(ct);
    ...
    public async Task GetTodaysPuzzle_DoesNotRepeatARecentlyServedExpression()
    {
        ...
        {
            var seeded = await NewService(seedCtx, today.AddDays(-1)).GetTodaysPuzzleAsync(ct);
